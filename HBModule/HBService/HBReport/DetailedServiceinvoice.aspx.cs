...
namespace HBReport
{
    public partial class DetailedServiceinvoice : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                var Name = Id[1].Split('@');
                string[] Ids = Name[1].Split('/');
                string[] Names = Ids[0].Split('/');
                string[] CkoutId = Ids[1].Split('/');
                string[] ChkiD = CkoutId[0].Split(',');
                command = new SqlCommand();
                ds = new DataSet();
                command.CommandText = "SearchDetailServiceInvoice_Help";
                ...
                command.Parameters.Add("@Str2", SqlDbType.NVarChar).Value = Names[0];
                command.Parameters.Add("@Id1", SqlDbType.Int).Value = ChkiD[0];// Convert.ToInt32(Id[1]);
                ...
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("") + @"\DetailedServiceinvoice.rdlc";
