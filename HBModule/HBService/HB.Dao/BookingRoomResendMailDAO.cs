...
namespace HB.Dao
{
    public class BookingRoomResendMailDAO
    {
        ...
        public DataSet Mail(int BookingId, string Email, string UsrEmail, User user)
        {
            UserData = " UserId:" + user.Id + ", UsreName:" + user.LoginUserName + ", ScreenName:'" + user.ScreenName +
             "', SctId:" + user.SctId + ", Service : BookingRoomResendMailDAO : Help --> Action Name - RoomBookingConfirmed" + ", ProcName:'" + StoredProcedures.BookingDtls_Help;
            ...
            command.Parameters.Add("@Action", SqlDbType.NVarChar).Value = "BookingStatus";
            command.Parameters.Add("@Str", SqlDbType.NVarChar).Value = "";
            command.Parameters.Add("@Id", SqlDbType.BigInt).Value = BookingId;
            ds = new WrbErpConnection().ExecuteDataSet(command, UserData);
            //
            if (ds.Tables[0].Rows[0][0].ToString() == "MMT")
            {
                ds = new DataSet();
                ds = new APIBookingResendMailDAO().Mail(BookingId, Email, UsrEmail, user);
                Err = "";
                dT.Rows.Add(Err);
                ds.Tables.Add(dT);
                return ds;
            ...
            else
            {
                ...
                catch (Exception ex)
                {
                    ...
                    log.ErrorLog(ex.Message + " --> Room Level Booking Resend Confirmation Mail --> " + message.Subject);
                    Err = ex.Message + " --> Room Resend Mail --> " + message.Subject;
                }
                if (Err != "")
                {
                    dT.Rows.Add(Err);
