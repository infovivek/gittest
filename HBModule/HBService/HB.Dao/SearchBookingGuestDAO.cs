...
namespace HB.Dao
{
    public class SearchBookingGuestDAO
    {
        String UserData;
        public DataSet Help(string[] data, User user)
        {
            ...
            command.CommandText = StoredProcedures.SearchBookingGuest_Help;
        ...
        public DataSet Search(string[] data, User user)
        {
            ...
            SqlCommand command = new SqlCommand();
            command.CommandText = StoredProcedures.SearchBookingGuest_Search;
            command.CommandType = CommandType.StoredProcedure;
            if (data[3] == null)
            {
                command.Parameters.Add("@Str1", SqlDbType.NVarChar).Value = "";
            ...
            if (data[8] == null)
            {
                command.Parameters.Add("@Id2", SqlDbType.NVarChar).Value = "";
            }
            else
            {
                command.Parameters.Add("@Id2", SqlDbType.BigInt).Value = data[8].ToString();
            }
            if (data[9] == "")
            {
                command.Parameters.Add("@Id3", SqlDbType.NVarChar).Value = 0;
            }
            else
            {
