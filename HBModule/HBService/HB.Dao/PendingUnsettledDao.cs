...
namespace HB.Dao
{
    public class PendingUnsettledDao
    {
        public DataSet Help(string[] data, User user)
        {
            string UserData = "UserId : " + user.Id + ", UsreName : " + user.LoginUserName + ", ScreenName : '" + user.ScreenName +
               "', SctId : " + user.SctId + ", Service :PendingUnsettled_Help ReportsDAO" + ", ProcName : '" + StoredProcedures.PendingUnsettled_Help;
            SqlCommand command = new SqlCommand();
            command.CommandText = StoredProcedures.PendingUnsettled_Help;
