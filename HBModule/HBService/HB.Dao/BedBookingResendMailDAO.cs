...
namespace HB.Dao
{
    public class BedBookingesendMailDAO
    {
        ...
        public DataSet Mail(int BookingId, string Email, string UsrEmail,User user)
        {
            ...
            if (PtyType == "MGH")
            {
                GuestDetailsTable1 =
                    ...
                    " <th style=\"background-color:#ccc; padding:6px 0px; border-right:1px solid #666;\"><p>Room No</p></th>" +
            ...
            catch (Exception ex)
            {
                ...
                log.ErrorLog(ex.Message + " --> Bed Level Booking Confirmation Resend Mail --> " + message.Subject);
                Err = ex.Message + " --> Bed Resend Mail --> " + message.Subject;
            }
            if (Err != "")
            {
                dT.Rows.Add(Err);
