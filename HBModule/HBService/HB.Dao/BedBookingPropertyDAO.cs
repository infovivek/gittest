...
namespace HB.Dao
{
    public class BedBookingPropertyDAO
    {
        string UserData = "";
        public DataSet Help(string[] data, User user)
        {
            UserData = " UserId:" + user.Id + ", UsreName:" + user.LoginUserName + ", ScreenName:'" + user.ScreenName +
             "', SctId:" + user.SctId + ", Service : BedBookingPropertyDAO : Help, " + ", ProcName:'" + StoredProcedures.BedBooking_Help; 
            SqlCommand command = new SqlCommand();
            command.CommandText = StoredProcedures.BedBooking_Help;
        ...
        public DataSet Save(string BedBookingProperty, User Usr, int BookingId)
        {
            UserData = " UserId:" + Usr.Id + ", UsreName:" + Usr.LoginUserName + ", ScreenName:'" + Usr.ScreenName +
             "', SctId:" + Usr.SctId + ", Service : BedBookingPropertyDAO : Save, " + ", ProcName:'" + StoredProcedures.BedBookingProperty_Insert; 
            DataSet ds = new DataSet();
            DataTable dTable = new DataTable("ERRORTBL");
            dTable.Columns.Add("Exception");
            XmlDocument document = new XmlDocument();
            BedBookingProperty BookPty = new BedBookingProperty();
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (document.SelectNodes("//GridXml")[i].Attributes["DiscountTariff"].Value == "")
                {
                    BookPty.DiscountTariff = 0;
                }
                else
                {
                    BookPty.DiscountTariff =
                        Convert.ToDecimal(document.SelectNodes("//GridXml")[i].Attributes["DiscountTariff"].Value);
                }
                ...
                {
                    BookPty.DiscountAllowed =
                        Convert.ToDecimal(document.SelectNodes("//GridXml")[i].Attributes["DiscountAllowed"].Value);
                }
                //
                SqlCommand command = new SqlCommand();
                if (BookPty.Id == 0)
                {
                    command.CommandText = StoredProcedures.BedBookingProperty_Insert;
                }
                else
                {
                ...
                command.Parameters.Add("@DiscountTariff", SqlDbType.Decimal).Value = BookPty.DiscountTariff;
