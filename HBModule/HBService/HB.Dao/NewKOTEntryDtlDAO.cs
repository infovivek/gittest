...
namespace HB.Dao
{
    public class NewKOTEntryDtlDAO
    {
        String UserData;
        public DataSet Save(string NewKOTEntryHdr, User Usr, int NewKOTEntryHdrId)
        {
            ...
            NewKOTEntryDtl NewKOTDtl = new NewKOTEntryDtl();
            ...
            //    NewKOTDtl.Quantity = Convert.ToInt32(document.SelectNodes("//ServiceXml")[i].Attributes["Quantity"].Value);
                ...
                if (NewKOTDtl.Id != 0)
                {
                    UserData = " UserId:" + Usr.Id + ", UsreName:" + Usr.LoginUserName + ", ScreenName:'" + Usr.ScreenName +
                        "', SctId:" + Usr.SctId + ", Service:PCHistroryDAO Help" + ", ProcName:'" + StoredProcedures.NewKOTEntryDtl_Update; 

                    command.CommandText = StoredProcedures.NewKOTEntryDtl_Update;
                    command.Parameters.Add("@Id", SqlDbType.BigInt).Value = NewKOTDtl.Id;
                }
                else
                {
                    UserData = " UserId:" + Usr.Id + ", UsreName:" + Usr.LoginUserName + ", ScreenName:'" + Usr.ScreenName +
                        "', SctId:" + Usr.SctId + ", Service:PCHistroryDAO Help" + ", ProcName:'" + StoredProcedures.NewKOTEntryDtl_Insert; 

                    command.CommandText = StoredProcedures.NewKOTEntryDtl_Insert;
                }
