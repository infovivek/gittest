...
IF @Action ='PageLoad'
    ...
    --GET DATA FROM BOOKING ROOM LEVEL CHECKIN DATE WISE  
     ...
     PropertyId,PropertyType,ApartmentId,CityId,PropertyName,CityName,RoomCaptured)   
            ...
            Select Distinct BookingCode,ClientId,ClientName,C.Id,RoomId,RoomName,Ag.FirstName,CONVERT(NVARCHAR,ChkInDt,103) ChkInDt,
		   ...
		   --Convert(date,Ag.ChkInDt,103)>= CONVERT(date,'01/09/2014',103)
			Group by C.id,s.HotalName,S.HotalId,ag.FirstName,Ag.ChkOutDt,RoomName,Ag.FirstName,S.City,C.CityId,C.ClientName,C.ClientId,
	...
	-- Select * from #BookingSelect where isnull(propertyName,'')=''
	-- return
	...
	CONVERT(DATE,CheckOutDt,103) BETWEEN CONVERT(DATE,@FromDate,103) AND CONVERT(DATE,@ToDate,103)
	 ...
	 -- CheckInDt, CheckOutDt, ApartmentId,Category,ServicePaymentMode,TariffPaymentMode,PropertyId,
	 --PropertyType,CityId,C.CityName,PropertyName ,RoomCaptured and TariffPaymentMode=@PayMode; 
...
--   group by BookingCode,ClientId,ClientName,BookingId,RoomId,RoomName,GuestName,CheckInDt,CheckOutDt,Occupancy,BookingLevel,Tariff,
--	 CheckInDt, CheckOutDt, ApartmentId,Category,ServicePaymentMode,TariffPaymentMode,PropertyId,PropertyType,CityId,C.CityName,PropertyName 
...
--Set @PrptyType='CPP'   where @PrptyType='Entity Preferred';
--Set @PrptyType='OTHER' where @PrptyType='Other Property';
--Set @PrptyType='GH'    where @PrptyType='Guest Houses';
If(@PrptyType='All')
BEGIN
		INSERT INTO #tempDat(ClientId,ClientName,NightCount,StayPersons,DayDiff ,TOTAL,PropertyName,CityName,
		TariffPaymentMode,CheckInDt,CheckOutDt ,CityId,PropertyId,PropertyType,BookingId)
...
BEGIN     
     ...
     GROUP BY   CityName,CityId  order by CityName   
end   
 ...
 order by CityName   
  if(@MonthWise=1)  
	 BEGIN   
		  ...
		  FROM #FrontEnd WHERE   TOTAL!=0 order by CityName --GROUP BY  PropertyName,PropertyId ,CityName,CityId   
	  END   
	 If(@MonthWise=3)  
	 Begin  
		   ...
		   NightCount StayPersons,StayPersons NightCount,TOTAL  
		   from #FrontEnd WHERE   TOTAL!=0  order by CityName    
	  End  
    If(@MonthWise=6)  
	 Begin  
