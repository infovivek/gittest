...
{
    public class BedBookingService:IBusinessService
    {
        public DataSet Save(string[] data, User user)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        DataSet ds_Property = new BedBookingPropertyBO().Save(data[3], user, BookingId);
                        ...
                        {
                            DataSet ds_Booked = new BedBookingPropertyAssignedGuestBO().Save(data[4], user, BookingId);
                            if (ds_Property.Tables["DBERRORTBL"].Rows.Count > 0)
                            {
                                dTable.Rows.Add(ds_Property.Tables["DBERRORTBL"].Rows[0][0].ToString());
                            }
        ...
        {
            return new BedBookingPropertyBO().Help(data, user);
        }
    }
}
