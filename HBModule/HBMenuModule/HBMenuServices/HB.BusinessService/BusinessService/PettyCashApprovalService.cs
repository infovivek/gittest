...
{
    public class PettyCashApprovalService : IBusinessService
    {
        public DataSet Save(string[] data, User user)
        {
            ...
            try
            {
                ...
                ds = new PettyCashApprovalHdrBO().Save(Hdrval, user);
                ...
                {
                    Int32 PettyCashApprovalHdrId = Convert.ToInt32(ds.Tables[0].Rows[0][0].ToString());
                    string PettyCashApprovalHdrRowId = ds.Tables[0].Rows[0][1].ToString();
                    string PettyCashApprovalHdr = data[2].ToString();
                    DataSet ds1 = new PettyCashApprovalDtlBO().Save(PettyCashApprovalHdr, user, PettyCashApprovalHdrId);
                    ...
                    else if (ds1.Tables[0].Rows[0][0].ToString() == "Need Valid Managers Approval")
                    {
                        dTable.Rows.Add("Need Valid Manager Approval");
                    }
                 }
            }
             catch (Exception Ex)
        ...
        {
            return new PettyCashApprovalHdrBO().HelpResult(data, user);
        }
    }
}
