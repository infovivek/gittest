...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_ExternalInvoiceReport_Help]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)

DROP PROCEDURE dbo.[SP_ExternalInvoiceReport_Help]
...
CREATE PROCEDURE dbo.[SP_ExternalInvoiceReport_Help]
(
@Action NVARCHAR(100),
@Id		BIGINT,
@Str1 NVARCHAR(100),
...
BEGIN
	...
	WHERE BT.GetType='Property' AND BT.PropertyType ='ExP'  AND
	...
	CONVERT(NVARCHAR(100),CONVERT(DATE,BillEndDate,103),110) AS BillEndDate,TotalAmount,Amount,FBAmount AS FoodAndBeverages,SAmount AS Service,
