...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_ProformainvoiceExtChkOutService_Bill]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_ProformainvoiceExtChkOutService_Bill]
GO 
/*=============================================
Author Name  : shameem
...
*******************************************************************************************************
-- =============================================*/
CREATE PROCEDURE [dbo].[SP_ProformainvoiceExtChkOutService_Bill]
...
--	CS.CheckOutHdrId =  @Id1
 ...
 set @ProType = (select top 1 PropertyType from WRBHBPreformaCheckOut where Id = @Id1 and IsActive = 1 and IsDeleted = 0)
 --select @ProType ;
	...
	SELECT @Food=SUM(ISNULL(D.ChkOutSerAmount,0)) FROM  WRBHBPreformaCheckOutServiceDtls D 
	WHERE D.IsActive=1 AND D.IsDeleted=0
    AND ltrim(D.TypeService)=ltrim('Food And Beverages')
    AND D.CheckOutServceHdrId=@Id1 
    
    SELECT @Laundry=SUM(ISNULL(D.ChkOutSerAmount,0)) FROM  WRBHBPreformaCheckOutServiceDtls D 
	WHERE D.IsActive=1 AND D.IsDeleted=0
    AND D.TypeService='Laundry'
    AND D.CheckOutServceHdrId=@Id1
    
    SELECT @Service=SUM(ISNULL(D.ChkOutSerAmount,0)) FROM  WRBHBPreformaCheckOutServiceDtls D 
	WHERE D.IsActive=1 AND D.IsDeleted=0
    ...
    FROM dbo.WRBHBPreformaCheckOutServiceHdr H
	...
	SELECT @ClientId=I.ClientId  FROM WRBHBPreformaCheckOut H
	...
	CessName NVARCHAR(MAX),HCessName NVARCHAR(MAX),PIInvoice NVARCHAR(100))
	...
	LaundryName,ServiceName,FoodBev,PropertyType,CessName,HCessName,PIInvoice)
    ...
    'Education Cess @2.00% ' CessName,'Secondary and Higher Education Cess @1.00%' HCessName,h.PIInvoice
	 
	from  WRBHBCheckInHdr d
	 join WRBHBPreformaCheckOut h on h.ChkInHdrId = d.Id and d.IsActive = 1 and d.IsDeleted = 0
	join WRBHBPreformaCheckOutServiceHdr CS on h.Id = cs.CheckOutHdrId and CS.IsActive = 1 and CS.IsDeleted= 0
	--join WRBHBCheckOutServiceDtls CSD on cs.Id = csd.CheckOutServceHdrId and CSD.IsActive = 1 and CSD.IsDeleted = 0
...
--	join WRBHBTaxMaster t on t.StateId=s.Id   
    ...
    ,h.PropertyType,CS.CreatedDate,h.PIInvoice--,t.LuxuryNo
...
--	join WRBHBTaxMaster t on t.StateId=s.Id   
   ...
   INSERT INTO #Prop2(GuestName ,Name,Stay ,Type,BookingLevel ,BillDate ,ClientName ,Id,InVoiceNo ,
    ...
    H.VATPer,h.RestaurantSTPer ,
    h.BusinessSupportST ,h.InVoiceNo,h.BillFromDate,h.BillEndDate,CS.ChkOutServiceNetAmount,h.PropertyType,
    CS.CreatedDate,h.PIInvoice--,t.LuxuryNo
