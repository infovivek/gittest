...
SET QUOTED_IDENTIFIER ON
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_PreformaInvoiceServiceHdr_Insert]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_PreformaInvoiceServiceHdr_Insert]
GO
/*=============================================
Author Name  : Anbu
...
*******************************************************************************************************
-- =============================================*/
CREATE PROCEDURE [dbo].[SP_PreformaInvoiceServiceHdr_Insert](
@CheckOutHdrId INT,
...
@BTC NVARCHAR(100);
SET @GuestName = (SELECT GuestName FROM WRBHBPreformaCheckOut WHERE Id = @CheckOutHdrId)
SET @PropertyType = (SELECT PropertyType FROM WRBHBPreformaCheckOut WHERE Id = @CheckOutHdrId)
SET @Direct = (SELECT Direct FROM WRBHBPreformaCheckOut WHERE Id = @CheckOutHdrId)
SET @BTC = (SELECT BTC FROM WRBHBPreformaCheckOut WHERE Id = @CheckOutHdrId)


 -- INSERT
INSERT INTO WRBHBPreformaCheckOutServiceHdr(CheckOutHdrId,ChkOutServiceAmtl,
...
SET @Id1=@@IDENTITY;
SELECT CheckOutHdrId as Id,Id RowId FROM WRBHBPreformaCheckOutServiceHdr WHERE Id=@Id1;
