...
SET QUOTED_IDENTIFIER ON
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_PreformaInvoiceServiceDtl_Insert]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_PreformaInvoiceServiceDtl_Insert]
GO
/*=============================================
Author Name  : Anbu
...
*******************************************************************************************************
-- =============================================*/
CREATE PROCEDURE [dbo].[SP_PreformaInvoiceServiceDtl_Insert](
...
@CreatedBy BIGINT,@Id BIGINT,@Quantity DECIMAL(27,2),@ServiceHdrId int)
...
@GuestName nvarchar(100),@ChkInHdrId INT;
 -- INSERT
INSERT INTO WRBHBPreformaCheckOutServiceDtls(CheckOutServceHdrId,ChkOutSerAction,
...
GETDATE(),1,0,NEWID(),@ProductId,@TypeService,@ServiceHdrId)


SET @Id1=@@IDENTITY;
SELECT Id,RowId FROM WRBHBPreformaCheckOutServiceDtls WHERE Id=@Id1;
...
--UPDATE WRBHBKOTDtls SET CheckOutServiceFlag = 1 WHERE HdrDate = CONVERT(Date,@ChkOutSerDate,103) AND  
...
--alter table WRBHBCheckOutServiceDtls add ServiceHdrId Bigint
--update WRBHBCheckOutServiceDtls set ServiceHdrId=0
