...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_LaundryServiceHdr_Help]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_LaundryServiceHdr_Help]
GO 
-- ===============================================================================
-- Author: Anbu
-- Create date:22-09-2014
-- ModifiedBy :          , ModifiedDate: 
-- Description:	LaundryServiceHdr_Help
-- =================================================================================
CREATE PROCEDURE [dbo].[SP_LaundryServiceHdr_Help]
...
If @Action ='Property'
	...
	-- To check intermediate flag = 1
			...
			--WHERE H.PropertyId = @Id AND H.IsActive=1 AND H.IsDeleted=0 AND B.CurrentStatus='CheckIn'
			--AND p.Category = 'Internal Property' AND 
			...
			--WHERE H.PropertyId = 2 AND H.IsActive=1 AND H.IsDeleted=0 AND B.CurrentStatus='CheckIn'
			--AND p.Category = 'Internal Property' AND 
			-- H.Id  IN(SELECT ChkInHdrId FROM WRBHBChechkOutHdr where IsActive = 1 and IsDeleted = 0 
			-- and isnull(IntermediateFlag,0)=1)
...
If @Action ='GuestName'
		...
		SELECT P.Id,ISComplimentary,(P.ProductName+'+'+SP.SubType) AS ProductName,PerQuantityprice,Enable,P.Id,
		TypeService
		FROM WRBHBContarctProductMaster P
