...
SET QUOTED_IDENTIFIER ON
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[Sp_PreformaInvoiceIntcheckout_Help]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[Sp_PreformaInvoiceIntcheckout_Help]
GO
-- ===============================================================================
-- Author: shameem
...
-- =================================================================================
CREATE PROCEDURE [dbo].[Sp_PreformaInvoiceIntcheckout_Help]
...
IF @Action='ServiceEntry'  
 BEGIN
	UPDATE WRBHBPreformaCheckOut SET  ServiceEntryFlag=1 where Id=@CheckInHdrId
 END
IF @Action='PageLoad'  
	BEGIN  
	...
	ELSE   
	BEGIN  
		SELECT TOP 1 CAST(CheckoutNo AS INT)+1 AS CheckoutNo FROM WRBHBPreformaCheckOut  
...
BEGIN  
		...
		--AND ISNULL(Flag,0)=1 AND ISNULL(Flag,0)=0 AND Status = 'CheckOut' )
...
If @BookingLevel = 'Room'   
	BEGIN  
		INSERT INTO #LEVEL(ChkInDate,ChkOutDate,TariffPaymentMode,ServicePaymentMode,BTCFile)  
		...
		TariffPaymentMode,ServicePaymentMode,BTCFilePath  
		from WRBHBBookingPropertyAssingedGuest  
...
If @BookingLevel = 'Apartment'  
 ...
 DECLARE @Roles NVARCHAR(100);
		...
		DECLARE @TariffPaymentMode NVARCHAR(100),@ServicePaymentMode NVARCHAR(100),@BTCFile NVARCHAR(MAX);  
		...
		SET @BTCFile =( SELECT TOP 1 BTCFile FROM #LEVEL)  
...
If @BookingLevel = 'Room'   
	BEGIN  
		INSERT INTO #LEVELs(ChkInDate,ChkOutDate,TariffPaymentMode,ServicePaymentMode,BTCFile)  
		--select top 1 CONVERT(nvarchar(100),min(ChkInDt),103),CONVERT(nvarchar(100),max(ChkOutDt),103) , 
		--TariffPaymentMode,ServicePaymentMode  
		--from WRBHBBookingPropertyAssingedGuest  
		--where GuestId = 29544 and BookingId = 4976 
...
If @BookingLevel = 'Apartment'  
	...
	END   
		...
		SELECT  @CheckInTime=ArrivalTime,@NewCheckInTimeType=TimeType,
		@NewCheckInDate=convert(nvarchar(100),Cast(ArrivalDate as DATE),103)	FROM WRBHBCheckInHdr  
		WHERE IsActive = 1 and IsDeleted = 0 and Id =@CheckInHdrId
	...
	END
		...
		SET @BTCFile =( SELECT TOP 1 BTCFile FROM #LEVELs) 
		--SELECT ClientName,Direct,BTC,Id From  WRBHBCheckInHdr    
		--WHERE Id=@CheckInHdrId AND IsActive=1 AND IsDeleted=0  
...
If @BookingLevel = 'Room'   
	BEGIN  
		INSERT INTO #LEVEL1(ChkInDate,ChkOutDate,TariffPaymentMode,ServicePaymentMode,BTCFile)  
...
-- this is chkin and chkout date after comes selected bill date
 ...
 --Type  
		...
		CREATE TABLE #PaymentMode(TariffPaymentMode NVARCHAR(100),ServicePaymentMode NVARCHAR(100),BTCFile NVARCHAR(MAX))  
...
If @BookingLevel = 'Room'   
	BEGIN  
		INSERT INTO #PaymentMode(TariffPaymentMode,ServicePaymentMode,BTCFile)  
...
If @BookingLevel = 'Apartment'  
	...
	--	DECLARE @TariffPaymentMode nvarchar(100),@ServicePaymentMode nvarchar(100);  
		...
		SET @BTCFile =( SELECT TOP 1 BTCFile FROM #PaymentMode)  
		--SELECT ClientName,Direct,BTC,Id From  WRBHBCheckInHdr    
		--WHERE Id=@CheckInHdrId AND IsActive=1 AND IsDeleted=0  
...
BEGIN
	...
	SELECT Property,Stay,ChkOutTariffAdays  AS Tariff,  
		...
		@BTCFiles AS BTCFile,Id From  WRBHBCheckInHdr    
		WHERE Id=@CheckInHdrId-- AND IsActive=1 AND IsDeleted=0  
		--Day Count  
...
---upto this paid with flag 0 is taken  for else part of checkout   
		...
		0.00 as ChkOutServiceNetAmount,  
		0.00 as OutStanding, CS.PaymentStatus  
		FROM WRBHBPreformaCheckOutServiceHdr CS  
		JOIN WRBHBPreformaCheckOut  CH ON CS.CheckOutHdrId=CH.Id --AND CH.IsActive=1 AND CH.IsDeleted=0  
      ...
      --Name  
		...
		SELECT DISTINCT Name FROM WRBHBPreformaCheckOut  
		...
		FROM WRBHBPreformaCheckOutServiceHdr H  
		JOIN WRBHBChechkOutHdr D ON H.CheckOutHdrId = D.Id --AND D.IsActive = 1 AND D.IsDeleted = 0  
		WHERE 
		--H.IsActive=1 AND H.IsDeleted=0 AND 
		D.Id=@PropertyId;  
